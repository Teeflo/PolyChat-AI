...
interface ChatStore {
  ...
  checkImageCompatibility: (runnableSessions: ChatSession[]) => {
    isCompatible: boolean;
    errorMessage?: string;
  };
  promoteTemporarySessions: (runnableSessions: ChatSession[], set: SetState) => ChatSession[];
  addUserMessageToSessions: (
    sessionsToProcess: ChatSession[],
    userMessage: Message,
    set: SetState
  ) => ChatSession[];
  ...
  createPlaceholderMessage: (sessionId: string) => {
    id: string;
    message: Message;
    ac: AbortController;
  };
  updateWithPlaceholder: (
    sessionId: string,
    placeholder: { id: string; message: Message; ac: AbortController },
    set: SetState
  ) => void;
  ...
  updateAfterAllResponses: (resolvedSessions: ChatSession[], set: SetState, get: GetState) => void;
...
export const useChat = create<ChatStore>((set, get) => ({
  ...
  initializeChat: () => {
    ...
    useSettings.subscribe((state) => {
      ...
      if (
        ...
        currentActiveSession.isTemporary &&
        state.selectedModel &&
        currentActiveSession.modelId === 'default'
      ) {
    ...
    useSettings.subscribe((state, prevState) => {
      ...
      if (
        ...
        !activeSessions[0].isTemporary
      ) {
        // Only update if not a temporary session
  ...
  setSessionModel: (sessionId: string, modelId: string) => {
    ...
    const updatedActive = activeSessions.map((s) =>
      s.id === sessionId ? { ...s, modelId, modelName: modelId, isTemporary: false } : s
    );
    const updatedAll = allSessions.map((s) =>
      s.id === sessionId ? { ...s, modelId, modelName: modelId, isTemporary: false } : s
    );
    const updatedSelected = selectedModels.map((id) =>
      activeSessions.find((s) => s.id === sessionId)?.modelId === id ? modelId : id
    );
    set({
      activeSessions: updatedActive,
      allSessions: updatedAll,
      selectedModels: updatedActive.map((s) => s.modelId) || updatedSelected,
    });
  },
  ...
  checkImageCompatibility: (
    runnableSessions: ChatSession[]
  ): { isCompatible: boolean; errorMessage?: string } => {
    const incompatibleModels = runnableSessions.filter(
      (session) => !isImageGenerationModel(session.modelId)
    );
    if (incompatibleModels.length > 0) {
      return {
        isCompatible: false,
        errorMessage: getImageGenerationError(incompatibleModels[0].modelId),
      };
    }
  ...
  getEffectiveSystemPrompt: (systemPrompt: string, tone: string): string | undefined => {
    ...
    return systemPrompt && systemPrompt.trim()
      ...
      : tonePrefix
        ? `${tonePrefix}Tu es un assistant IA utile.`
        : undefined;
  },
  ...
  getContextMessages: async (
    ...
    if (ragEnabled) {
      try {
        ...
        // Fallback to normal context if RAG fails
      }
    }
  ...
  createPlaceholderMessage: (
    sessionId: string
  ): { id: string; message: Message; ac: AbortController } => {
  ...
  updateWithPlaceholder: (
    ...
    set((state) => ({
      activeSessions: state.activeSessions.map((s: ChatSession) =>
        s.id === sessionId ? { ...s, messages: [...s.messages, placeholder.message] } : s
      ),
      allSessions: state.allSessions.map((s: ChatSession) =>
        s.id === sessionId ? { ...s, messages: [...s.messages, placeholder.message] } : s
      ),
      abortControllers: { ...state.abortControllers, [sessionId]: placeholder.ac },
    }));
  },
          ...
          s.id === session.id
            ? {
                ...
                messages: s.messages.map((m: Message) =>
                  m.id === placeholderId ? { ...m, content: responseContent, streaming: false } : m
                ),
              }
            : s
        ),
...
Prompt original: "${getMessageText(userMessage.content)}"`,
        ...
        activeSessions: state.activeSessions.map((s: ChatSession) =>
          s.id === session.id
            ? {
                ...s,
                messages: s.messages.map((m: Message) =>
                  m.id === placeholderId ? { ...m, content: errorContent, streaming: false } : m
                ),
              }
            : s
        ),
  ...
  handleTextStreaming: async (
    ...
    await streamAIResponse(
        ...
        set((state) => ({
          ...
          streamingProgress: {
            ...
            [session.id]: state.streamingProgress[session.id]
              ? {
                  ...state.streamingProgress[session.id],
                  chars: state.streamingProgress[session.id].chars + delta.length,
                  lastUpdate: now,
                }
      ...
      effectiveSystem,
      get().abortControllers[session.id]
    );
      ...
      activeSessions: state.activeSessions.map((s: ChatSession) =>
        s.id === session.id
          ? {
              ...
              messages: s.messages.map((m: Message) =>
                m.id === placeholderId
                  ? { ...m, content: `Erreur: ${errorMessage}`, streaming: false }
                  : m
              ),
            }
          : s
  ...
  processSessionResponse: async (
    ...
    const {
      ...
      ac,
    } = get().createPlaceholderMessage(session.id);
    get().updateWithPlaceholder(
      session.id,
      { id: placeholderId, message: placeholderMessage, ac },
      set
    );
    ...
    try {
      ...
      if (isImageRequest && isImageGenerationModel(session.modelId)) {
        return await get().handleImageGeneration(
          session,
          userMessage,
          apiKey,
          placeholderId,
        ...
        return await get().handleTextStreaming(
          contextMessages,
          apiKey,
          session,
          placeholderId,
  ...
  updateAfterAllResponses: (resolvedSessions: ChatSession[], set: SetState) => {
  ...
  sendNotification: (resolvedSessions: ChatSession[], notificationsEnabled: boolean) => {
    ...
    try {
      const hasImages = resolvedSessions.some((session) => {
        ...
        return (
          messageText.includes('![Image générée]') ||
  ...
  sendMessageToAll: async (content: string) => {
    ...
    const sessionsToProcess = get().promoteTemporarySessions(runnableSessions, set);
    get().addUserMessageToSessions(sessionsToProcess, userMessage, set);
    const updatedSessions = get().activeSessions;

    const promises = updatedSessions.map((session) =>
      get().processSessionResponse(
        session,
        content,
        apiKey,
        tone || 'neutre',
  ...
  regenerateMessage: async (sessionId: string, messageId: string) => {
    ...
    try {
          ...
          activeSessions: state.activeSessions.map((s) =>
            s.id === sessionId
              ? {
                  ...s,
                  isLoading: false,
                  error: 'Impossible de régénérer sans message utilisateur précédent.',
                }
              : s
          ),
          isAnyLoading: false,
      ...
      const placeholderMessage: Message = {
        ...createMessage('assistant', '…', session.modelId),
        streaming: true,
      };
      regenPlaceholderId = placeholderMessage.id;
      const msgsWithoutOld = [
        ...conversationHistory.filter((m) => m.id !== messageId),
        placeholderMessage,
      ];
      const ac = new AbortController();
      ...
      set((state) => ({
        activeSessions: state.activeSessions.map((s) =>
          s.id === sessionId ? { ...s, messages: msgsWithoutOld } : s
        ),
              ...
              s.id === sessionId
                ? {
                    ...
                    messages: s.messages.map((m) =>
                      m.id === regenPlaceholderId
                        ? { ...m, content: responseContent, streaming: false }
                        : m
                    ),
                  }
                : s
...
Le système a essayé plusieurs modèles et méthodes, mais la régénération a échoué.`,
      ...
      const finalUpdateSession = (s: ChatSession) =>
        s.id === sessionId
          ? {
              ...s,
      ...
      if (notificationsEnabled) {
        // notify('PolyChat AI', 'Régénération terminée');
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Erreur lors de la régénération';
      const regenIdErr = regenPlaceholderId;
      const errorUpdateSession = (s: ChatSession) =>
        s.id === sessionId
          ? {
              ...s,
              messages: s.messages.map((m) =>
                m.id === regenIdErr
                  ? { ...m, streaming: false, content: `Erreur: ${errorMessage}` }
                  : m
              ),
              isLoading: false,
              error: errorMessage,
