    ...
    <div className={`flex items-center gap-2.5 select-none ${className}`}>
      ...
      >
        ...
        <g opacity={isWhite ? 0.9 : 0.8}>
          {/* Left Face */}
          <path d="M50 50 L18 68 V32 L50 14 V50Z" fill={fillUrl} opacity="0.8" />
          {/* Right Face (Chat Tail) */}
          <path d="M50 50 L82 32 V68 L50 86 V50Z" fill={fillUrl} opacity="0.6" />
          {/* Top Face (Brain) */}
          <path d="M50 50 L82 32 L50 14 L18 32 L50 50Z" fill={fillUrl} opacity="1" />
        </g>
      ...
      {showText && (
        <div className="flex flex-col leading-none">
          <span
            className={`font-bold text-xl tracking-tight flex items-center ${isWhite ? 'text-white' : 'text-slate-800 dark:text-white'}`}
          >
            Poly
            <span
              className={`ml-0.5 ${isWhite ? 'text-blue-200' : 'bg-clip-text text-transparent bg-gradient-to-r from-violet-500 to-blue-500'}`}
            >
              Chat
            </span>
          </span>
